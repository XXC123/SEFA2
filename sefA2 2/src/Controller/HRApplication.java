...
public class HRApplication {
	...
	//import job data from database to arraylist jobs
	public static void importJobinfo() {
		File jobInfo = new File("Database/jobInfo.txt");		
		//ArrayList<Job> jobList=new ArrayList<Job>();
		if (jobInfo.exists()) {
			try {
				Scanner input = new Scanner(new FileReader(jobInfo));
				while (input.hasNextLine()){
					StringTokenizer jobFile = new StringTokenizer(
							input.nextLine(), ",");
					
					String name = jobFile.nextToken();
					String classType=jobFile.nextToken();
					String start = jobFile.nextToken();
					String end = jobFile.nextToken();
					String time = jobFile.nextToken();
					int numOfStaff = Integer.valueOf(jobFile.nextToken());
					double payRate = Double.valueOf(jobFile.nextToken());
					
					Job job=new Job(name,classType,start,end,time,numOfStaff,payRate);
					jobs.add(job);
				}
			}catch (IOException e) {
			System.out.println(e.getMessage());
	...
	public static void createCourseEvent() {
		Scanner stdin = new Scanner(System.in);
		String name, classType,start, end, time;
		int numOfStaff;
		double payRate;
		...
		name = stdin.next();
		System.out.print("Class type of the job: ");
		classType = stdin.next();
		...
		// Creates new job
		Job j = new Job(name,classType, start, end, time, numOfStaff, payRate);
		jobs.add(j);
